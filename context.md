...
- Personal context uses fixed categories: ordinary `preferences`, `projects-commitments`, and `user-rules`; sensitive `identity`, `people`, `education-work`, `owned-items`, and `important-dates`. Explicit ordinary writes are direct and replay-safe. Sensitive writes, sensitive corrections, and every forget operation require a one-time approval bound to the reviewed record version.
- SQLite is authoritative for active context and provenance revisions; private `0600` JSON mirrors under `SageData/context/<category>/` make the managed registry inspectable. Forgetting removes the mirror and redacts every retained value while keeping content-free audit tombstones.
- Every Sage model request receives only bounded confirmed global or query-relevant context. Unconfirmed conversation content never becomes memory. Sage may suggest an exact `/remember category | key | value` command, but only an explicit user request can persist it.
