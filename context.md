...
- Model role contracts are versioned in `prompts/sage-system.md` and `prompts/iris-system.md`. Sage's dispatcher loads its contract on every model request; Iris's contract will be applied by the vision dispatch path rather than at server startup because OpenAI-compatible system prompts are request-scoped.
